<?php 
    ...
    if(isset($_POST['registerBtn'])){
        ...
        //check if error array is empty, if yes process and insert data
            ...
            try{
                //Create SQL insert 
                $sqlInsert = "INSERT INTO users (username,fullname, email, password,rno,yos,address,mob,cgpa,hname)
                                VALUES (:username,:fullname, :email, :password,:rno,:yos,:address,:mob,:cgpa,:hname)";

                //sanitise data
                $statement = $db->prepare($sqlInsert);

                //Add data into the db 
                $statement->execute(array(':username' => $username, ':email' => $email, ':password' => $hashed_password,':rno'=>$rno,'yos'=>$yos,':address'=>$address,':fullname'=>$fullname,':mob'=>$mob,':cgpa'=>$cgpa,':hname'=>$hname));
